...
ASSETS_PATH = OUTPUT_PATH / Path(r"/Users/kostaskaplanis/Desktop/BookEat/src/assets/frame0")
...
class MyApplication:
    def __init__(self, master):
        ...
        self.master.iconphoto(True, PhotoImage(file="./assets/login/bookeat_icon.png"))
        # Calculate the position to center the window
        window_position_x = self.master.winfo_screenwidth() // 2 - 600  # Half of window width (500 / 2)
        window_position_y = self.master.winfo_screenheight() // 2 - 450  # Half of window height (800 / 2)

        # Set the window geometry with the calculated position
        self.master.geometry("1237x856+{}+{}".format(window_position_x, window_position_y))
        self.admin = Admin()
        self.create_widgets()
    ...
    def add_book(self):
        if any(entry.get() == "" for entry in [self.entry_1, self.entry_2, self.entry_3, self.entry_4]):
            tkinter.messagebox.showerror("Error", "You need to fill all fields.")
            return
        ...
        for entry in [self.entry_1, self.entry_2, self.entry_3, self.entry_4]:
